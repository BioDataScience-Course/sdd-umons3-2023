...
<!--# TODO: Mettre en évidence les différentes catégories de métriques: multiclasses, 2 classes et composite (F-score et balanced accuracy) + parler dans une section optionnelle des méthodes micro- et macroaverage -->
...
En taxonomie assistée par ordinateur, nous cherchons donc à accélérer le dépouillement de gros échantillons en identifiant de manière automatique (ou semi-automatique) la faune et/ou la flore qui le compose. L'approche la plus souvent employée est l'analyse d'image pour mesurer des caractères visuels sur ces photographies. Il s'agit donc d'une classification sur base de critères morphologiques. Les organismes qui composent la faune et/ou la flore sont les items et les critères morphologiques sont les attributs. Vous avez certainement déjà rêvé d'avoir quelqu'un qui fait vos identifications à votre place... pourquoi pas votre ordinateur ?!
...
Ces outils ont très certainement éveillé votre curiosité. Imaginez qu'à l'avenir il ne sera peut plus utile d'apprendre la classification des bourdons. Un ordinateur pourra le faire à votre place. Il y a néanmoins certaines conditions d'application à satisfaire préalablement afin de pouvoir utiliser efficacement le "machine learning" :
...
<!-- TODO: this is on an old branch, not a problem, but should we place it elsewhere? (with the cheatsheets?) -->
...
if (exists("assignment2"))
  assignment2("C01Ga_metrics", part = NULL,
    ...
    course.urls = c(
      'S-BIOG-025'         = "https://classroom.github.com/a/I3apm73-"),
    course.starts = c(
      'S-BIOG-025'         = !"{W[5]+1} 08:00:00"),
    course.ends = c(
      'S-BIOG-025'         = !"{W[5]+5} 23:59:59"),
    term = "Q1", level = 4, n = 4,
...
Ce jeu de données contient `r ncol(penguins)` variables et `r nrow(penguins)` individus. La variable que nous cherchons à prédire ici est `species` (variable réponse de type `factor`). Trois espèces de manchots sont étudiées :
...
Le set d'apprentissage a `r nrow(penguins_train)` items et le set de test a `r nrow(penguins_test)` individus. Les proportions entre espèces sont ici bien mieux respectée qu'un échantillonnage purement au hasard, non stratifié sur l'espèce (proportions identiques à l'unité près).
...
if (exists("assignment"))
  assignment("C01Ib_lda", part = NULL,
    ...
    course.urls = c(
      'S-BIOG-025'         = "https://classroom.github.com/a/SGc7abMA"),
    course.starts = c(
      'S-BIOG-025'         = !"{W[5]+4} 10:00:00"),
    course.ends = c(
      'S-BIOG-025'         = !"{W[5]+4} 12:30:00"),
    term = "Q1", level = 3,
